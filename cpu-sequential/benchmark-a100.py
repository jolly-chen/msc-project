...
base_header = "iter,env,nthreads,nbins,bulksize,input,edges,tfindbin,tfill,tstats,ttotal"

def run_benchmark(f, n, environs, nthreads, bulksizes, nbins, input_files, output_file=""):
    ...
    with open(output_file, "a") as file_handler:
        for iter in range(n):
            for ei, e in enumerate(environs):
                for ti, nt in enumerate(nthreads):
                   for nbi, nb in enumerate(nbins):
                       for bi, b in enumerate(bulksizes):
                           for edges in ["", "-e"]:
                               procs = []
                               write_result = '-w' if iter == 0 and bi == 0 and ti == 0 else ''
                               for nif, ipf in enumerate(input_files):
                                   input_file = f"{input_folder}/{ipf}"
                                   stem = Path(ipf).stem
                                   cmd = f""
                                   arg = f"-b{b} -h{nb} -f{input_file} {edges} {write_result} -t{nt}"
                                   print(f"{cmd} {f} {arg}")
...
#                               for p, stem in procs:
   ...
   #                                    print(output)
                                       ...
                                       file_handler.write(
                                           f"{iter},{e},{nt},{nb},{b},{stem},{'True' if edges != '' else 'False'},{','.join(times)}\n"
                                       )
                                       file_handler.flush()
...
#        64,
    ...
    f = "/data/jolly/msc-project-sm80/benchmarks/histond_benchmark"
    if len(sys.argv) > 1:
        output_file = sys.argv[1]
    else:
        output_file = f"{time.strftime('%Y%m%d-%H%M%S')}"

    output_folder = "/data/jolly/msc-project-sm80/cpu-sequential/a100-cpu"
    os.makedirs(output_folder, exist_ok=True)
    print(f"writing results to {output_folder}/{output_file}...")

    run_benchmark(
        f, n, environs, num_threads, bulksizes, nbins, input_files, f"{output_folder}/{output_file}"
    )

    print(f"Finished writing results to {output_folder}/{output_file}...")
